module.exports = {
  intent: 'revert',
  matcher: /^(верни|вернуть|верни назад|верни как было|забудь|удали(ть)?|убрать|убери|сотри|стереть|отмени(ть)?|отмена|не то|неправильно|не так) ?(последн(ее|ий|ие|их))? ?(запись|это|его|удаление|очистку|изменени(е|я|й)|добавление)?$/i,
